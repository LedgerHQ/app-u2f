...
struct ble_state_u2f_s G_io_ble_u2f;
...
unsigned char custom_mac[6];

unsigned char DEFAULT_NAME[DEVICE_NAME_LENGTH+1]; //+1 for AD_TYPE
...
const uint8_t u2f_serviceRevisionBitfield[16] = {0xbb, 0x23, 0xd6, 0x7e, 0xba, 0xc9,
                                            0x2f, 0xb4, 0xee, 0xec, 0xaa, 0xde,
                                            0xf4, 0xff, 0xd0, 0xf1};
const uint8_t u2f_serviceRevisionBitfield_value[1] = { 0xc0 }; // 1.1 and 1.2
...
bool add_DIS_Service(void) {
  tBleStatus ret;

  ret = aci_gatt_add_service(UUID_TYPE_16, dis_service_uuid, PRIMARY_SERVICE, 10,
                          &G_io_ble_u2f.dis_service_handle); /* original is 9?? */
  if (ret != BLE_STATUS_SUCCESS) {
    return false;
  }

  ret = aci_gatt_add_char(
      G_io_ble_u2f.dis_service_handle, UUID_TYPE_16, dis_manufacturer_name,
      BLE_CHUNK_LENGTH_B, CHAR_PROP_READ /*|CHAR_PROP_WRITE_WITHOUT_RESP*/,
      ATTR_PERMISSION_NONE, 0, 10, 1, &G_io_ble_u2f.dis_manufacturer_name_handle);
  if (ret != BLE_STATUS_SUCCESS) {
    return false;
  }
  ret = aci_gatt_update_char_value(
      G_io_ble_u2f.dis_service_handle, G_io_ble_u2f.dis_manufacturer_name_handle, 0,
  ...
  ret = aci_gatt_add_char(
      G_io_ble_u2f.dis_service_handle, UUID_TYPE_16, dis_model_number,
      BLE_CHUNK_LENGTH_B, CHAR_PROP_READ /*|CHAR_PROP_WRITE_WITHOUT_RESP*/,
      ATTR_PERMISSION_NONE, 0, 10, 1, &G_io_ble_u2f.dis_model_number_handle);
  if (ret != BLE_STATUS_SUCCESS) {
    return false;
  }
  ret = aci_gatt_update_char_value(
      G_io_ble_u2f.dis_service_handle, G_io_ble_u2f.dis_model_number_handle, 0,
  ...
  ret = aci_gatt_add_char(
      G_io_ble_u2f.dis_service_handle, UUID_TYPE_16, dis_hw_rev_string,
      BLE_CHUNK_LENGTH_B, CHAR_PROP_READ /*|CHAR_PROP_WRITE_WITHOUT_RESP*/,
      ATTR_PERMISSION_NONE, 0, 10, 1, &G_io_ble_u2f.dis_hw_rev_string_handle);
  if (ret != BLE_STATUS_SUCCESS) {
    return false;
  }
  ret = aci_gatt_update_char_value(
      G_io_ble_u2f.dis_service_handle, G_io_ble_u2f.dis_hw_rev_string_handle, 0,
  ...
  ret = aci_gatt_add_char(
      G_io_ble_u2f.dis_service_handle, UUID_TYPE_16, dis_fw_rev_string,
      BLE_CHUNK_LENGTH_B, CHAR_PROP_READ /*|CHAR_PROP_WRITE_WITHOUT_RESP*/,
      ATTR_PERMISSION_NONE, 0, 10, 1, &G_io_ble_u2f.dis_fw_rev_string_handle);
  if (ret != BLE_STATUS_SUCCESS) {
    return false;
  }
  ret = aci_gatt_update_char_value(
      G_io_ble_u2f.dis_service_handle, G_io_ble_u2f.dis_fw_rev_string_handle, 0,
...
bool add_U2F_Service(void) {
  tBleStatus ret;

  ret = aci_gatt_add_service(UUID_TYPE_16, u2f_service_uuid, PRIMARY_SERVICE, 10, (uint16_t*)&G_io_ble.service_handle); /* original is 9?? */
  if (ret != BLE_STATUS_SUCCESS) {
    return false;
  }
  ...
  /* TX */
  #ifdef DYNAMIC_MTU
    ret =  aci_gatt_add_char(G_io_ble.service_handle, UUID_TYPE_128, u2f_charUuidTX, MAX_MTU_SIZE-3, CHAR_PROP_NOTIFY/*|CHAR_PROP_READ*/, ATTR_PERMISSION_NONE /*ATTR_PERMISSION_ENCRY_READ*/, 0,
                 10, 1, (uint16_t*)&G_io_ble.tx_characteristic_handle);
  #else
    ret =  aci_gatt_add_char(G_io_ble.service_handle, UUID_TYPE_128, u2f_charUuidTX, BLE_CHUNK_LENGTH_B, CHAR_PROP_NOTIFY/*|CHAR_PROP_READ*/, ATTR_PERMISSION_NONE /*ATTR_PERMISSION_ENCRY_READ*/, 0,
                 10, 1, (uint16_t*)&G_io_ble.tx_characteristic_handle);
  #endif
  ...
  /* RX */
  #ifdef DYNAMIC_MTU
    ret =  aci_gatt_add_char(G_io_ble.service_handle, UUID_TYPE_128, u2f_charUuidRX, MAX_MTU_SIZE-3, CHAR_PROP_WRITE/*|CHAR_PROP_WRITE_WITHOUT_RESP*/, /*ATTR_PERMISSION_NONE*/ ATTR_PERMISSION_AUTHEN_WRITE, GATT_NOTIFY_WRITE_REQ_AND_WAIT_FOR_APPL_RESP,
                 10, 1, (uint16_t*)&G_io_ble.rx_characteristic_handle);
  #else
    ret =  aci_gatt_add_char(G_io_ble.service_handle, UUID_TYPE_128, u2f_charUuidRX, BLE_CHUNK_LENGTH_B, CHAR_PROP_WRITE/*|CHAR_PROP_WRITE_WITHOUT_RESP*/, /*ATTR_PERMISSION_NONE*/ ATTR_PERMISSION_AUTHEN_WRITE, GATT_NOTIFY_WRITE_REQ_AND_WAIT_FOR_APPL_RESP,
                 10, 1, (uint16_t*)&G_io_ble.rx_characteristic_handle);
  #endif
  ...
  ret = aci_gatt_add_char(
      ...
      BLE_CHUNK_LENGTH_B, CHAR_PROP_READ /*|CHAR_PROP_WRITE_WITHOUT_RESP*/,
      PERMISSION_U2F_READ, 0, 10, 1, &G_io_ble_u2f.u2f_controlpointlength_handle);
  if (ret != BLE_STATUS_SUCCESS) {
    return false;
  }

  ret = aci_gatt_update_char_value(
      G_io_ble.service_handle, G_io_ble_u2f.u2f_controlpointlength_handle, 0,
...
// Not defined in 1.2
  ...
  ret = aci_gatt_add_char(
      ...
      BLE_CHUNK_LENGTH_B, CHAR_PROP_READ /*|CHAR_PROP_WRITE_WITHOUT_RESP*/,
      PERMISSION_U2F_READ, 0, 10, 1, &G_io_ble_u2f.u2f_servicerevision_handle);
  if (ret != BLE_STATUS_SUCCESS) {
    return false;
  }

  ret = aci_gatt_update_char_value(
      G_io_ble.service_handle, G_io_ble_u2f.u2f_servicerevision_handle, 0,
  ...
  /* serviceRevisionBitfield */  

  ret = aci_gatt_add_char(
      G_io_ble.service_handle, UUID_TYPE_128, u2f_serviceRevisionBitfield,
      BLE_CHUNK_LENGTH_B, CHAR_PROP_READ | CHAR_PROP_WRITE_WITHOUT_RESP /*|CHAR_PROP_WRITE_WITHOUT_RESP*/,
      PERMISSION_U2F_READ | PERMISSION_U2F_WRITE, 0, 10, 1, &G_io_ble_u2f.u2f_servicerevisionbitfield_handle);
  if (ret != BLE_STATUS_SUCCESS) {
    return false;
  }  

  ret = aci_gatt_update_char_value(
      G_io_ble.service_handle, G_io_ble_u2f.u2f_servicerevisionbitfield_handle, 0,
      sizeof(u2f_serviceRevisionBitfield_value), u2f_serviceRevisionBitfield_value);
  if (ret != BLE_STATUS_SUCCESS) {
    return false;
  }
...
void BLE_diversify_name_address() {
  ...
  */
  ...
  unsigned int i = os_serial(se_serial, sizeof(se_serial));
  ...
  snprintf(DEFAULT_NAME+1, sizeof(DEFAULT_NAME)-1, "Nano X U2F %02X%02X", se_serial[0], se_serial[1]);
  DEFAULT_NAME[0]=AD_TYPE_SHORTENED_LOCAL_NAME;
...
unsigned int BLE_make_discoverable(const char* discovered_name) {
  tBleStatus ret ;
  BOOL error = FALSE;
  ...
  ScanRespData[0] = 3;
  ScanRespData[1] = AD_TYPE_16_BIT_SERV_UUID;
  memcpy(&ScanRespData[2], u2f_service_uuid, 2);
  // Add slave prefererd connexion interval in scan response data
  ScanRespData[4] = 5;
  ScanRespData[5] = AD_TYPE_SLAVE_CONN_INTERVAL;
  ScanRespData[6] = CONN_MS_TO_UNIT(BLE_SLAVE_CONN_INTERVAL_MIN_MS); // *1.25ms
  ScanRespData[7] = 0;
  ScanRespData[8] = CONN_MS_TO_UNIT(BLE_SLAVE_CONN_INTERVAL_MAX_MS); // *1.25ms
  ScanRespData[9] = 0;
  ...
  // Add 16 bits service uuid list in scan response
  ret = hci_le_set_scan_response_data(10, ScanRespData);
  if (ret != BLE_STATUS_SUCCESS) {
    return 1;
  }
 ...
 */
unsigned int BLE_setup(unsigned char *discovered_name) {
  tBleStatus ret;
  BOOL error = FALSE;
  ...
  //G_io_u2f.bleChunkSize = G_io_ble.ble_chunk_length;
...
void BLE_power_mac(unsigned char powered, const char *discovered_name, const char *mac) {
  ...
  if (mac == NULL) {
    memset(custom_mac, 0, sizeof(custom_mac));
  }
  else {
    memmove(custom_mac, mac, sizeof(custom_mac));
  }
...
void BLE_power(unsigned char powered, const char *discovered_name) {
  return BLE_power_mac(powered, discovered_name, NULL);
}
...
void BLE_protocol_recv(unsigned char data_length, unsigned char* att_data) {  
  ...
  u2f_transport_received(&G_io_u2f, att_data, data_length, U2F_MEDIA_BLE);
...
char BLE_protocol_send(unsigned char* response_apdu, unsigned short response_apdu_length) {
  ...
  BLE_send(G_io_ble.tx_characteristic_handle, response_apdu, response_apdu_length);
  // release the seal (f)
  G_io_ble.apdu_transport_lock = 0;
  ...
  case EVT_VENDOR:
    {
      ...
      switch(blue_evt->ecode){
        ...
        case EVT_BLUE_ATT_EXCHANGE_MTU_RESP:
        {
          ...
          //G_io_u2f.bleChunkSize = G_io_ble.ble_chunk_length;
          mtuValue[0] = G_io_ble.ble_chunk_length >> 8;
          mtuValue[1] = G_io_ble.ble_chunk_length;
          ret = aci_gatt_update_char_value(
            G_io_ble.service_handle, G_io_ble_u2f.u2f_controlpointlength_handle, 0,
            sizeof(mtuValue), mtuValue);
          break;
        }
      #endif
