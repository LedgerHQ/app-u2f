...
from fido2.ctap1 import ApduError, Ctap1, RegistrationData, SignatureData
...
class LedgerCtap1(Ctap1):
    ...
    def __init__(
        self, ctap_device: CtapDevice, device: Device, navigator: Navigator, debug: bool = False
    ):
    ...
    def confirm(self):
        ...
        self.navigator.navigate(instructions, screen_change_after_last_instruction=False)
    ...
    def wait_for_return_on_dashboard(self, dismiss: bool = False):
        if dismiss and self.ledger_device.type == DeviceType.STAX:
            ...
            self.navigator.navigate(
                [NavInsID.USE_CASE_STATUS_DISMISS], screen_change_before_first_instruction=True
            )
    ...
    def register(
        ...
        if check_screens:
            ...
            self.navigator.navigate_and_compare(
                root, test_name, instructions, screen_change_after_last_instruction=False
            )
        elif instructions:
            self.navigator.navigate(instructions, screen_change_after_last_instruction=False)
