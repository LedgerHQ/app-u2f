...
const unsigned char DEFAULT_NAME_CONST[] = {AD_TYPE_COMPLETE_LOCAL_NAME, 'x',' ','U','2', 'F', '\0'};
...
void BLE_make_discoverable(const char* discovered_name) {
  ...
  */
  ret = aci_gap_set_discoverable(ADV_IND, 
                                  ...
                                  sizeof(u2f_serviceUUIDList), u2f_serviceUUIDList,
                                  // low power leverage
...
#define PERMISSION_U2F_READ ATTR_PERMISSION_AUTHOR_READ | ATTR_PERMISSION_ENCRY_READ
#define PERMISSION_U2F_WRITE ATTR_PERMISSION_AUTHOR_WRITE | ATTR_PERMISSION_ENCRY_WRITE
...
tBleStatus Add_U2F_Service(void)
{
  ...
  ret =  aci_gatt_add_char(G_io_ble.service_handle, UUID_TYPE_128, u2f_charUuidTX, BLE_CHUNK_LENGTH_B, CHAR_PROP_NOTIFY, PERMISSION_U2F_READ, 0,
                           16, 1, &G_io_ble.tx_characteristic_handle);
  if (ret != BLE_STATUS_SUCCESS) goto fail;

  PRINTF("Notify added\n");
  
  ret =  aci_gatt_add_char(G_io_ble.service_handle, UUID_TYPE_128, u2f_charUuidRX, BLE_CHUNK_LENGTH_B, CHAR_PROP_WRITE/*|CHAR_PROP_WRITE_WITHOUT_RESP*/, PERMISSION_U2F_WRITE, GATT_NOTIFY_WRITE_REQ_AND_WAIT_FOR_APPL_RESP,
  ...
  ret =  aci_gatt_add_char(G_io_ble.service_handle, UUID_TYPE_128, u2f_controlPointLength, BLE_CHUNK_LENGTH_B, CHAR_PROP_READ/*|CHAR_PROP_WRITE_WITHOUT_RESP*/, PERMISSION_U2F_READ, 0,
  ...
  ret =  aci_gatt_add_char(G_io_ble.service_handle, UUID_TYPE_16, u2f_serviceRevision, BLE_CHUNK_LENGTH_B, CHAR_PROP_READ/*|CHAR_PROP_WRITE_WITHOUT_RESP*/, PERMISSION_U2F_READ, 0,
...
void BLE_protocol_recv(unsigned char data_length, unsigned char* att_data) {
  ...
  u2f_transport_handle(u2fServiceReference, att_data, data_length, U2F_MEDIA_BLE);
