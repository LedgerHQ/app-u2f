...
__attribute__((weak)) int u2f_get_cmd_msg_data_length(const uint8_t* buffer,
                                                      uint16_t length) {
    ...
    if (length == APDU_MIN_HEADER + 1) {
        // Short encoding, with next byte either Le or Lc with the other one
        // omitted There is no way to tell so no way to check the value but
        // anyway the data length is 0

        // Support this particular short encoding APDU as Fido Conformance Tool
        // v1.7.0 is using it even though spec requires that short encoding
        // should not be used over HID.
        return 0;
    }
    ...
    // so extended encoding with Lc field present, optionally Le (2B) is present
    // too
    ...
    if ((APDU_MIN_HEADER + LONG_ENC_LC_SIZE + dataLength != length) &&
        (APDU_MIN_HEADER + LONG_ENC_LC_SIZE + dataLength + LONG_ENC_LE_SIZE !=
         length)) {
        return -1;
    }
...
typedef struct u2f_reg_resp_base_t {
    ...
    uint8_t key_handle[CREDENTIAL_MINIMAL_SIZE];  // We generate fix size key
                                                  // handles
...
static void u2f_compute_enroll_response_hash(u2f_reg_resp_base_t* reg_resp_base,
    ...
    cx_hash(&hash.header, 0, globals_get_u2f_data()->application_param,
            sizeof(globals_get_u2f_data()->application_param), NULL, 0);
    cx_hash(&hash.header, 0, globals_get_u2f_data()->challenge_param,
            sizeof(globals_get_u2f_data()->challenge_param), NULL, 0);
    cx_hash(&hash.header, 0, reg_resp_base->key_handle, key_handle_length, NULL,
            0);
    cx_hash(&hash.header, CX_LAST, reg_resp_base->user_key,
            sizeof(reg_resp_base->user_key), data_hash, CX_SHA256_SIZE);
}
...
static int u2f_prepare_enroll_response(void) {
    ...
    // Generate private and public key and fill public key
    {
        ...
        if (crypto_generate_public_key(&private_key, reg_resp_base->user_key,
                                       CX_CURVE_SECP256R1) <= 0) {
        ...
        key_handle_length = credential_wrap(
            globals_get_u2f_data()->application_param,
            globals_get_u2f_data()->nonce, &private_key,
            reg_resp_base->key_handle, sizeof(reg_resp_base->key_handle));
    ...
    if (key_handle_length == sizeof(reg_resp_base->key_handle)) {
        ...
        memmove(G_io_apdu_buffer + offset, ATTESTATION_CERT,
                sizeof(ATTESTATION_CERT));
        offset += sizeof(ATTESTATION_CERT);

        // Prepare signature
        uint8_t data_hash[CX_SHA256_SIZE];
        u2f_compute_enroll_response_hash(reg_resp_base, key_handle_length,
                                         data_hash);
...
exit:
    if (result < 0) {
        result =
            u2f_fill_status_code(SW_PROPRIETARY_INTERNAL, G_io_apdu_buffer);
    }
...
static void u2f_compute_sign_response_hash(u2f_auth_resp_base_t* auth_resp_base,
    ...
    cx_hash(&hash.header, 0, auth_resp_base->counter,
            sizeof(auth_resp_base->counter), NULL, 0);
    cx_hash(&hash.header, CX_LAST, globals_get_u2f_data()->challenge_param,
            sizeof(globals_get_u2f_data()->challenge_param), data_hash,
            CX_SHA256_SIZE);
}
...
static int u2f_prepare_sign_response(void) {
    ...
    u2f_auth_resp_base_t* auth_resp_base =
        (u2f_auth_resp_base_t*)G_io_apdu_buffer;
...
/* Register Flow */
UX_STEP_CB(ux_register_flow_0_step, pbb, u2f_callback_confirm(NULL),
           {
               &C_icon_validate_14,
               "Register",
               verifyName,
           });
UX_STEP_NOCB(ux_register_flow_1_step, bnnn_paging,
             {
                 .title = "Identifier",
                 .text = verifyHash,
             });
UX_STEP_CB(ux_register_flow_2_step, pbb, u2f_callback_cancel(NULL),
           {
               &C_icon_crossmark,
               "Abort",
               "register",
...
UX_FLOW(ux_register_flow, &ux_register_flow_0_step, &ux_register_flow_1_step,
        &ux_register_flow_2_step, FLOW_LOOP);

/* Authenticate Flow */
UX_STEP_CB(ux_login_flow_0_step, pbb, u2f_callback_confirm(NULL),
           {
               &C_icon_validate_14,
               "Login",
               verifyName,
           });
UX_STEP_NOCB(ux_login_flow_1_step, bnnn_paging,
             {
                 .title = "Identifier",
                 .text = verifyHash,
             });
UX_STEP_CB(ux_login_flow_2_step, pbb, u2f_callback_cancel(NULL),
           {
               &C_icon_crossmark,
               "Abort",
               "login",
...
UX_FLOW(ux_login_flow, &ux_login_flow_0_step, &ux_login_flow_1_step,
        &ux_login_flow_2_step, FLOW_LOOP);
...
static void u2f_prompt_user_presence(bool enroll,
...
static void u2f_handle_apdu_enroll(unsigned char* flags, unsigned short* tx,
    ...
    // Backup challenge and application parameters to be used if user accept the
    // request
    memmove(globals_get_u2f_data()->challenge_param, reg_req->challenge_param,
            sizeof(reg_req->challenge_param));
    memmove(globals_get_u2f_data()->application_param,
            reg_req->application_param, sizeof(reg_req->application_param));

    globals_get_u2f_data()->user_presence_request_type =
...
static void u2f_handle_apdu_sign(unsigned char* flags, unsigned short* tx,
                                 uint32_t data_length) {
    uint8_t* nonce;
    // Parse request base and check length validity
    u2f_auth_req_base_t* auth_req_base =
        (u2f_auth_req_base_t*)(G_io_apdu_buffer + OFFSET_DATA);
    ...
    uint8_t* key_handle =
        G_io_apdu_buffer + OFFSET_DATA + sizeof(u2f_auth_req_base_t);
    if (data_length !=
        sizeof(u2f_auth_req_base_t) + auth_req_base->key_handle_length) {
        return u2f_send_error(SW_WRONG_LENGTH, tx);
    }
    ...
    switch (G_io_apdu_buffer[OFFSET_P1]) {
        ...
        case P1_U2F_OPTIONAL_USER_PRESENCE:  // proof of user presence is always
                                             // required (1.2)
            sign = true;
            break;
        default:
    ...
    if (credential_unwrap(auth_req_base->application_param, key_handle,
                          auth_req_base->key_handle_length, &nonce) < 0) {
        return u2f_send_error(SW_WRONG_DATA, tx);
    }

    // If we only check user presence, get rid of the private key and answer
    // immediately
    if (!sign) {
        return u2f_send_error(SW_CONDITIONS_NOT_SATISFIED, tx);
    }

    // Backup nonce, challenge and application parameters to be used if user
    // accept the request
...
void handleApdu(unsigned char* flags, unsigned short* tx,
