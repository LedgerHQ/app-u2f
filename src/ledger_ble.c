...
#if defined(HAVE_BLE) && (defined(TARGET_BLUE) || defined(TARGET_ARAMIS))
...
void BLE_make_discoverable(const char *discovered_name) {
  ...
  */
  ...
  Name ret = aci_gap_update_adv_data(0, NULL); if (ret) { THROW(EXCEPTION);
  }
  */
  ...
  Adv_Filter_Policy, Local_Name_Length, Local_Name, Service_Uuid_Length,
  Service_Uuid_List, Slave_Conn_Interval_Min, Slave_Conn_Interval_Max
  */
...
void BLE_setup(unsigned char *discovered_name) {
  ...
  // limit exchanges and data security to limit power, moreover apdus are public
  // (f)
  //#if 0
...
#if 0
#define PERMISSION_U2F_READ ATTR_PERMISSION_NONE
#define PERMISSION_U2F_WRITE ATTR_PERMISSION_NONE
#endif
...
tBleStatus Add_U2F_Service(void) {
  ...
  ret = aci_gatt_add_char(G_io_ble.service_handle, UUID_TYPE_128,
                          u2f_charUuidTX, BLE_CHUNK_LENGTH_B, CHAR_PROP_NOTIFY,
                          PERMISSION_U2F_READ, 0, 16, 1,
  ...
  ret = aci_gatt_add_char(
      ...
      PERMISSION_U2F_WRITE, GATT_NOTIFY_WRITE_REQ_AND_WAIT_FOR_APPL_RESP, 16, 1,
  ...
  case EVT_VENDOR: {
    ...
    case EVT_BLUE_GATT_ATTRIBUTE_MODIFIED: {
      ...
      if (evt->attr_handle == G_io_ble.tx_characteristic_handle + 2) {
        // tx char handle attribute is triggered, the client registered to
    ...
    case EVT_BLUE_GATT_WRITE_PERMIT_REQ: {
      ...
      if (evt->attr_handle == G_io_ble.rx_characteristic_handle + 1) {
        ...
        // Accept it before processing its data to avoid deadlock in spi access
        // within attribute modified
        ...
        if (G_io_ble.client_link_established) {
          ...
          //  - reset
          //  - DFU
          //  - apdu transport (to deliver the SE, whom will afterwards drive
          //  the screen and request a pin code
